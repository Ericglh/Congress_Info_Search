...
<body>
	...
	function validate(){
		...
		if(document.getElementById("userinput").value.replace(/^\s+|\s+$/g,"") == ""){
			emptyfield += "keyword";
			valid = false;
		}else{
	...
	function clear(){
		document.getElementById('keyword')[0].value="Keyword*";
	...
	<div align="center">
		<form method="GET" name="form1">
			<fieldset style="width: 300px;margin: 0px;padding: 0px;">
				...
				<label id = "keyword">
				<?php 
					if(isset($_GET['Database'])&&isset($_GET['state'])&&trim($_GET['state']) == ""&&!isset($_GET['clear'])){
	...
	<?php
		...
		if(isset($_GET['bioguide_id'])){
			...
			if(sizeof($json->results) == 0){
				...
				foreach($json->results as $jr){
					...
					if($jr->short_title == null){
						$resShow .= "<tr align=\"center\"><td width=\"200px\" style=\"text-align:left;\">Bill Title</td><td width=\"200px\" style=\"text-align:left;\">NA</td></tr>";
					}else{
			...
			if(isset($_GET['Database']) && $_GET['Database'] == "legislators" && isset($_GET['chamber']) && isset($_GET['state']) &&trim($_GET['state']) != ""){
				...
				//input is the fullname of a legislator
					...
					if($len == 1){
						...
						//$apiQuery = "http://congress.api.sunlightfoundation.com/".$_GET['Database']. "?chamber=" .$_GET['chamber'] ."&query=" . $fname . "%20" . $lname ."&" .$apiKey;
						//$apiQuery = "http://congress.api.sunlightfoundation.com/".$_GET['Database']. "?chamber=" .$_GET['chamber'] ."&query=" . $fname . "&query=" . $lname ."&" .$apiKey;
				...
				if(sizeof($json->results) == 0){
					...
					foreach($json->results as $ln){
						...
						$secondUrl = "congress.php" . "?bioguide_id=" . $ln->bioguide_id . "&Database=" . $_GET['Database']. "&chamber=" .$_GET['chamber']. "&state=" .$_GET['state'].  "&" .$apiKey;
			...
			}else if(isset($_GET['Database']) && $_GET['Database'] == "committees" && trim($_GET['state']) != ""){
				...
				echo $resShow;
			}else if(isset($_GET['Database']) && $_GET['Database'] == "bills" && trim($_GET['state']) != ""){
				$keyword = "Bill ID*";
				$apiKey = "apikey=d19367a89106435ead658d7b683091f3";
				$apiQuery = "";
				...
				if(sizeof($json->results) == 0){
					...
					foreach($json->results as $jr){
						$secondUrl = "congress.php/" . "?bill_id=" . $jr->bill_id . "&chamber=" . $jr->chamber . "&Database=" . $_GET['Database']. "&state=". $_GET['state'];
				...
				echo $resShow;
			}else if(isset($_GET['Database']) && $_GET['Database'] == "amendments" && trim($_GET['state']) != ""){
	...
	?>
<noscript>Noscript! Aru you kidding me?</noscript>
</body>
</html>